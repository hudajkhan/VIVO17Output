...
public class FreemarkerHttpServlet extends VitroHttpServlet  {
    ...
    protected void doResponse(VitroRequest vreq, HttpServletResponse response, 
        ...
        try {
			...
			} else if (values instanceof JSONResponseValues) {
				doJSON(vreq, response, values);
			}
    ...
    /** This is to enable the profile page to be displayed in JSON - we could probably refactor code
     * to be more consistent or enable additional formats as well.
     * ?Convert the map to JSON? somehow?
     * @param request
     * @param response
     * @param values
     * @throws IOException
     */
    protected void doJSON(HttpServletRequest request, HttpServletResponse response, ResponseValues values) 
            throws IOException {
            
            String mediaType = values.getContentType().getMediaType();
            response.setContentType(mediaType);
            //Probably need another mechanism, or could include getJSON in the interface
             JSONResponseValues jr = (JSONResponseValues) values;       
           
                //json-ld is not supported by jena v2.6.4
                try {   
                   String jsonString = jr.getJSON();
                   PrintWriter writer = response.getWriter();
                   writer.write(jsonString);
                   writer.close();
                } catch (Exception e) {
                   throw new IOException("Could not convert from to JSON", e);
                }           
